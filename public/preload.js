...
window.services = {
  ...
  executeCopy: e => {
    ...
    if (window.rubick.hideMainWindow(), 1 === t.size) {
      ...
      if ("image" === e[0].type && 1 === e.length) {
        var bitmap = fs.readFileSync(e[0].value);
        return window.rubick.copyImage(`data:image/png;base64,${new Buffer(bitmap).toString('base64')}`)
      }
    }
